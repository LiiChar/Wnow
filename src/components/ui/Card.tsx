...
function Card(props: ComponentProps<'div'> & { size?: 'default' | 'sm' }) {
	...
	return (
		<div
			data-slot='card'
			class={cn(
				'ring-foreground/0 bg-card text-card-foreground gap-4 overflow-hidden rounded-xl py-4 text-sm ring-1 has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl group/card flex flex-col border border-border',
				local.class,
			)}
			{...rest}
		/>
